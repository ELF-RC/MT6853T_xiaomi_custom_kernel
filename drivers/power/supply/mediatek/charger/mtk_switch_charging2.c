...
static void swchg_select_charging_current_limit(struct charger_manager *info)
{
	...
	if (is_typec_adapter(info)) {
		...
		chr_err("[FIX_DBG] pd_type=%d pdc=%d pe40_in=%d pe40_chg=%d\n", info->pd_type, mtk_pdc_check_charger(info), info->data.pe40_single_charger_input_current, info->data.pe40_single_charger_current);
		pdata->input_current_limit =
				info->data.ac_charger_input_current;
		pdata->charging_current_limit =
				info->data.ac_charger_current;
...
bool get_charging_call_state(void) { return false; }

int pdpm_is_charge_pump_enable(void) { return 0; }
