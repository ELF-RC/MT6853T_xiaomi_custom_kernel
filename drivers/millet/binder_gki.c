...
void mi_binder_transaction_handler(void * data, struct binder_proc * target_proc, struct binder_proc * proc,
	struct binder_thread * thread, struct binder_transaction_data * tr, bool is_reply)
{
	if (is_reply) {
...
static int __init init_binder_gki(void)
{
	...
	register_trace_android_vh_binder_transaction(mi_binder_transaction_handler, NULL);
