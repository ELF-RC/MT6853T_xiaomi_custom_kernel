...
static const char * const NAME_ARRAY[MILLET_TYPES_NUM] = {
	"NULL",
	"SIG",
	"BINDER",
	"BINDER_STAT",
...
static void recv_handler(struct sk_buff *skb)
{
	...
	if (payload->src_port != MILLET_USER_ID) {
		pr_err("src_port %lx invalid! from %d\n",
		       payload->src_port, from);
...
static int __init millet_init(void)
{
	...
	/* Register sub-module hooks AFTER socket & locks are ready */
	millet_pkg_setup();
	millet_hs_setup();
