	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_PRESENT:
		val->intval = (mtk_chg->chg_type != CHARGER_UNKNOWN) ? 1 : 0;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		val->intval = battery_get_vbus() * 1000; /* mV -> uV */
		break;
	case POWER_SUPPLY_PROP_INPUT_CURRENT_NOW:
	{
		struct charger_device *chg = get_charger_by_name("primary_chg");
		unsigned int uA = 0;
		if (chg && !charger_dev_get_input_current(chg, &uA))
			val->intval = uA; /* uA */
		else
			val->intval = 0;
		break;
...
static void plug_in_out_handler(struct chg_type_info *cti, bool en, bool ignore)
{
	...
	/* 拔出时立即更新 chg_online 并通知 userspace */
	if (!en) {
		...
		if (chg_psy) {
			struct mt_charger *mtk_chg = power_supply_get_drvdata(chg_psy);
			if (mtk_chg) {
				mtk_chg->chg_online = false;
